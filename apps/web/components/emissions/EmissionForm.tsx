  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await apiFetch('/api/v1/emission-records', token!, {
        method: 'POST',
        body: JSON.stringify({ ...form, companyId, activityData: Number(form.activityData), emissionFactor: Number(form.emissionFactor) }),
      })
  ...
  return (
    <form onSubmit={submit} className="space-y-4 max-w-lg">
      <div>
        ...
        <select value={form.scope} onChange={(e) => update('scope', e.target.value as '1' | '2' | '3')} className="mt-1 block w-full border rounded p-2">
